...
import {
  getCharacter,
  getAbility,
  createCharacterAction, updateCharacterAction,
  createCharacterSpell, updateCharacterSpell,
  createCharacterCantrip, updateCharacterCantrip,
} from "@/lib/actions";
...
function AddAbilityPageContent() {
  ...
  const abilityType: AbilityType = rawType === "spell" ? "spell" : rawType === "cantrip" ? "cantrip" : "action";
  ...
  useEffect(() => {
    const load = async () => {
      ...
      try {
        ...
        if (abilityId) {
          ...
          if (Number.isNaN(parsedAbilityId)) { setHeaderError("Invalid ability id."); setHeaderLoading(false); return; }
          const abilityData = await getAbility(abilityType, parsedAbilityId) as Record<string, unknown> | null;
          if (abilityData) {
            setIsEditMode(true);
            setName(String(abilityData.name ?? ""));
            setScaling(String(abilityData.scaling ?? ""));
            setUsageTiming(String(abilityData.usage_timing ?? "action"));
            setDamage(String(abilityData.damage ?? ""));
            setDescription(String(abilityData.description ?? ""));
            if (abilityType === "spell") setSpellLevel(String(abilityData.spell_level ?? 1));
    ...
    void load();
  }, [abilityId, characterId, abilityType, session]);
  ...
  const handleSave = async () => {
    ...
    if (abilityType === "action") {
      restoreOnToSave = restoreOnMode === "another" ? (restoreOnCustom.trim() || null) : restoreOnMode;
      if (charges.trim()) {
        const parsed = Number.parseInt(charges, 10);
        if (!Number.isInteger(parsed) || parsed < 1) { setSaveError("Charges must be a positive integer."); return; }
    ...
    if (abilityType === "spell") {
      ...
      if (!Number.isInteger(parsed) || parsed < 1 || parsed > 9) { setSaveError("Spell level must be between 1 and 9."); return; }
    ...
    if (abilityType === "action") { basePayload.restore_on = restoreOnToSave; basePayload.charges = chargesToSave; basePayload.curr_charges = chargesToSave; }
    if (abilityType === "spell") { basePayload.spell_level = spellLevelToSave; }

    try {
      if (abilityId) {
        const parsedAbilityId = Number.parseInt(abilityId, 10);
        if (Number.isNaN(parsedAbilityId)) { setSaveError("Invalid ability id."); setIsSaving(false); return; }
        if (abilityType === "action") await updateCharacterAction(parsedAbilityId, basePayload);
        else if (abilityType === "spell") await updateCharacterSpell(parsedAbilityId, basePayload);
        else await updateCharacterCantrip(parsedAbilityId, basePayload);
      } else {
        if (abilityType === "action") await createCharacterAction(parsedCharacterId, basePayload);
        else if (abilityType === "spell") await createCharacterSpell(parsedCharacterId, basePayload);
        else await createCharacterCantrip(parsedCharacterId, basePayload);
      }
    ...
    <main className="min-h-screen bg-zinc-50 px-4 py-6 text-zinc-900">
      <div className="mx-auto flex min-h-[calc(100vh-3rem)] w-full max-w-5xl flex-col">
          ...
          <div className="mt-4 grid gap-4 md:grid-cols-2">
            <label className="space-y-1.5 md:col-span-2">
              ...
              <input type="text" value={name} onChange={(e) => setName(e.target.value)}
                placeholder={abilityType === "spell" ? "Fireball" : abilityType === "cantrip" ? "Fire Bolt" : "Second Wind"}
            ...
            <label className="space-y-1.5">
              <span className="text-xs font-semibold uppercase tracking-wide text-zinc-500">Usage Timing</span>
              <select value={usageTiming} onChange={(e) => setUsageTiming(e.target.value)}
            ...
            <label className="space-y-1.5">
              <span className="text-xs font-semibold uppercase tracking-wide text-zinc-500">Damage</span>
              <input type="text" value={damage} onChange={(e) => setDamage(e.target.value)} placeholder="1d10 fire"
                className="w-full rounded-lg border border-zinc-300 bg-zinc-50 px-3 py-2 text-sm font-medium text-zinc-900 outline-none ring-zinc-900 placeholder:text-zinc-400 focus:ring-2" />
            </label>

            <label className="space-y-1.5">
              <span className="text-xs font-semibold uppercase tracking-wide text-zinc-500">Scaling / DC</span>
              <input type="text" value={scaling} onChange={(e) => setScaling(e.target.value)} placeholder="DC 15 CON save"
                className="w-full rounded-lg border border-zinc-300 bg-zinc-50 px-3 py-2 text-sm font-medium text-zinc-900 outline-none ring-zinc-900 placeholder:text-zinc-400 focus:ring-2" />
            </label>

            {abilityType === "spell" && (
              <label className="space-y-1.5">
                <span className="text-xs font-semibold uppercase tracking-wide text-zinc-500">Spell Level</span>
            ...
            {abilityType === "action" && (
              <>
                <label className="space-y-1.5">
                  <span className="text-xs font-semibold uppercase tracking-wide text-zinc-500">Restore On</span>
                  <select value={restoreOnMode}
                    onChange={(e) => setRestoreOnMode(e.target.value === "long_rest" ? "long_rest" : e.target.value === "another" ? "another" : "short_rest")}
                    className="w-full rounded-lg border border-zinc-300 bg-zinc-50 px-3 py-2 text-sm font-medium text-zinc-900 outline-none ring-zinc-900 focus:ring-2">
                ...
                {restoreOnMode === "another" && (
                  <label className="space-y-1.5">
                    <span className="text-xs font-semibold uppercase tracking-wide text-zinc-500">Restore On (Custom)</span>
                    <input type="text" value={restoreOnCustom} onChange={(e) => setRestoreOnCustom(e.target.value)} placeholder="e.g. Dawn"
                      className="w-full rounded-lg border border-zinc-300 bg-zinc-50 px-3 py-2 text-sm font-medium text-zinc-900 outline-none ring-zinc-900 placeholder:text-zinc-400 focus:ring-2" />
                  </label>
                )}
                <label className="space-y-1.5">
                  <span className="text-xs font-semibold uppercase tracking-wide text-zinc-500">Charges (optional)</span>
                  <input type="number" min="1" step="1" value={charges} onChange={(e) => setCharges(e.target.value)} placeholder="Leave empty if unlimited"
                    className="w-full rounded-lg border border-zinc-300 bg-zinc-50 px-3 py-2 text-sm font-medium text-zinc-900 outline-none ring-zinc-900 placeholder:text-zinc-400 focus:ring-2" />
                </label>
              </>
            )}
        ...
        <div className="mt-auto flex items-center justify-between gap-3 pt-6">
          <button type="button" onClick={() => router.push(`/edit/abilities-and-items?characterId=${characterId ?? ""}`)}
            className="rounded-lg bg-zinc-900 px-5 py-2.5 text-sm font-semibold text-white transition hover:bg-zinc-700">
            Back
          </button>
          <button type="button" onClick={handleSave} disabled={isSaving || headerLoading || !character}
